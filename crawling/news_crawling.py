from selenium import webdriver
from selenium.webdriver.common.by import By
from selenium.webdriver.chrome.service import Service
from webdriver_manager.chrome import ChromeDriverManager
import time
from bs4 import BeautifulSoup
import csv  # csv 라이브러리 임포트

# Chrome WebDriver 자동 설치
try:
    service = Service(ChromeDriverManager().install())
    driver = webdriver.Chrome(service=service)
except Exception as e:
    print(f"WebDriver 설치 및 실행 오류: {e}")
    exit()

# URL 접속
url = "https://news.naver.com/breakingnews/section/101/258"
driver.get(url)

# 페이지 로딩 대기
time.sleep(2)

# '기사 더보기' 버튼을 5번 클릭하는 루프
for _ in range(50):
    try:
        more_button = driver.find_element(By.CLASS_NAME, 'section_more_inner')
        more_button.click()
        time.sleep(2)  # 새로운 기사 로딩 대기
    except Exception:
        print("더 이상 '더보기' 버튼이 없습니다. 크롤링을 종료합니다.")
        break

# 모든 기사가 로딩된 후, BeautifulSoup으로 HTML 파싱
soup = BeautifulSoup(driver.page_source, 'html.parser')

# 뉴스 기사 정보 추출
news_items = soup.find_all('li', class_='sa_item')

# 데이터를 저장할 리스트
all_news_data = []

if not news_items:
    print("뉴스 기사를 찾을 수 없습니다. HTML 구조가 변경되었을 수 있습니다.")
else:
    for item in news_items:
        title_element = item.find('a', class_='sa_text_title')

        if title_element:
            try:
                link = title_element['href']
                title = title_element.find('strong').get_text(strip=True)

                # 딕셔너리 형태로 데이터 저장
                all_news_data.append({
                    '제목': title,
                    '링크': link
                })
            except (KeyError, AttributeError):
                # href 속성이나 strong 태그가 없는 경우 건너뛰기
                continue

# CSV 파일로 저장
file_name = 'naver_news.csv'
fields = ['제목', '링크']  # CSV 파일의 헤더

with open(file_name, 'w', newline='', encoding='utf-8-sig') as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames=fields)
    writer.writeheader()  # 헤더 작성
    writer.writerows(all_news_data)  # 데이터 작성

print(f"\n총 {len(all_news_data)}개의 기사를 {file_name} 파일에 저장했습니다.")

# 브라우저 닫기
driver.quit()